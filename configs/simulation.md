...
**故意不在此暴露的危险项**：`orn_max_hz`（嗅觉）、`jon_max_hz`（机械感受）保持默认 `0`。实测连续积分下强驱这两大群感觉神经元会触发**全脑自持递归活动「锁死」**（`active` 飙到 ~5500，去刺激后回不来；ORN 低至 ~8 Hz 即触发）。本项目转向/前进/梳理走直驱 DN 句柄、进食走糖 → MN9，均无需它们。如确需嗅觉可视化，可在本组写 `orn_max_hz`（`from_dict` 会接受），但请知悉锁死风险。
